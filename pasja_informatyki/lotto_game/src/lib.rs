                ...
                match lotto_numer_arr[i] {
                    Some(_)  => {
                        if value > 1 && value < 49 {
                            ...
                            return Err("Lotto number generation failed. The number is out of range. ")
                        }
                    },
                    None     => return Err("Generation of lotto set failed. Number in set is unexpected empty. "),
                }
            }
            Err(e)  => { 
                let error_message = format!("Generation of lotto set failed due to an error. Additionally: {}", e);
...
pub fn generate_lotto_set_output(lotto_set_result: Result<[Option<u8>; 6], &'static str>) -> [u8; 6] {
...
mod tests {
    ...
    fn test_generate_lotto_set_output() {
        let set_result: Result<[Option<u8>; 6], &'static str> = Ok([Some(1), Some(2), Some(3), Some(4), Some(5), Some(6)]);
